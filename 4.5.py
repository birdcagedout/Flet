...
class WatcherThread(Thread):
	...
	def run(self):
		...
		# 4.3: 엑셀파일 다운로드시 title: '다운로드 사유 입력' ==> 입력 + tab 2번 + space
		...
		while True:
			...
			if title == '다운로드 사유 입력':
				...
				self.k.release(pynputKeyboard.Key.tab)
				time.sleep(0.05)
				self.k.press(pynputKeyboard.Key.space)
	...
	def start_btn(e: ft.ControlEvent):
		...
		if car_system_launched == False:
			dlg_alert_carsystem = ft.AlertDialog(
				modal=True,
				title=ft.Row([
					ft.Icon(ft.icons.WARNING_ROUNDED, color=ft.colors.AMBER_600, size=75),
					ft.Text("실행 전에   ", size=23, weight=ft.FontWeight.BOLD),
					#ft.Icon(ft.icons.WARNING_ROUNDED, color=ft.colors.AMBER_600, size=35),
				], spacing=0, alignment=ft.MainAxisAlignment.CENTER),
				title_padding=ft.padding.only(top=24, left=0, right=0, bottom=0),

				content=ft.Text("자동차 관리정보시스템을\n먼저 실행해주세요.", size=16),
				content_padding=ft.padding.only(top=20, left=34, right=34, bottom=24),

				actions=[ft.TextButton("돌아가기", on_click=dlg_alert_carsystem)],
				actions_padding=ft.padding.only(top=0, left=34, right=34, bottom=20),
				actions_alignment=ft.MainAxisAlignment.END,
			)
		...
		if choice == None:
			dlg_alert_noneselected = ft.AlertDialog(
				...
				title=ft.Row([
					ft.Icon(ft.icons.WARNING_ROUNDED, color=ft.colors.AMBER_600, size=75),
					ft.Text("조회사유는    ", size=23, weight=ft.FontWeight.BOLD),
					#ft.Icon(ft.icons.WARNING_ROUNDED, color=ft.colors.AMBER_600, size=35),
				], spacing=0, alignment=ft.MainAxisAlignment.CENTER),
				title_padding=ft.padding.only(top=24, left=0, right=0, bottom=0),
